...
physics_compulsory([phy3211, phy3212, phy3213, phy3214]).

as_subject([T], M) :- course_unit_credits(T, M).
as_subject( [H|T], M) :- course_unit_credits(H, M1), as_subject(T,M2), M is M1+M2.

physics_subject([H|T]) :- as_subject([H|T], M), M>5, phy_compulsory_req([H|T], physics_compulsory(A).


member(H, [H|T]).
member(X, [H|T]) :- member(X, T).

Phy_compulsory_req([], A).
phy_compulsory_req([H|T], physics_compulsory(A) :- phy_compulsory_req(T, A), member(H,A).
